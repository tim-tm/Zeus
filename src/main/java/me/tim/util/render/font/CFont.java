...
public class CFont {
    ...
    public CFont(final Font font, final boolean antiAlias, final int charOffset, final boolean allChar, int yAddon) {
        this.font = font;
    ...
    public CFont(final Font font, final boolean antiAlias, final boolean allChar, int yAddon) {
        this.font = font;
        this.antiAlias = antiAlias;
        charOffset = 6;
        setupTexture(antiAlias, allChar, yAddon);
    ...
    public void drawString(final String text, double x, double y, final Color color, final boolean shadow) {
        ...
        RenderUtil.glColor(shadow ? new Color(0.05F, 0.05F, 0.05F, color.getAlpha() / 255F) : color);
    ...
    public void drawString(final String text, double x, double y, final int color, final boolean shadow) {
        ...
        glPushMatrix();
        GlStateManager.bindTexture(texID);
        RenderUtil.glColor(new Color(color));
        final int size = text.length();
        for (int indexInString = 0; indexInString < size; indexInString++) {
            final char character = text.charAt(indexInString);
            if (character < chars.length && character >= 0) {
    ...
    public void drawStringRaw(final String text, double x, double y, final int color, final boolean shadow) {
        x -= 3;
        y -= 1;
