...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        EventPreMotion eventPreMotion = new EventPreMotion(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.lastReportedPosX, this.lastReportedPosY, this.lastReportedPosZ, this.rotationYaw, this.rotationPitch, this.lastReportedYaw, this.lastReportedPitch, this.onGround);
        eventPreMotion.call();
        if (eventPreMotion.isCancelled()) return;
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(eventPreMotion.getX(), eventPreMotion.getY(), eventPreMotion.getZ(), eventPreMotion.getYaw(), eventPreMotion.getPitch(), eventPreMotion.isOnGround()));
                }
                else if (flag2)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(eventPreMotion.getX(), eventPreMotion.getY(), eventPreMotion.getZ(), eventPreMotion.isOnGround()));
                }
                else if (flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(eventPreMotion.getYaw(), eventPreMotion.getPitch(), eventPreMotion.isOnGround()));
                }
                else
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer(eventPreMotion.isOnGround()));
                }
            }
            else
            {
        ...
        EventPostMotion post = new EventPostMotion(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround);
        post.call();
        this.posX = post.getX();
        this.posY = post.getY();
        this.posZ = post.getZ();
        this.rotationYaw = post.getYaw();
        this.rotationPitch = post.getPitch();
    }
    ...
    public void sendChatMessage(String message)
    {
        if (message.startsWith(".")) {
            for (Command command : this.mc.zeusClient.commandManager.getCommands()) {
                for (String name : command.getNames()) {
                    if (String.format(".%s", name).equalsIgnoreCase(message.split(" ")[0])) {
