...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        if (this == Statics.getPlayer()) {
            ...
            if (Math.abs(x - eventMove.getX()) > 0) {
                x = eventMove.getX();
            }

            if (Math.abs(y - eventMove.getY()) > 0) {
                y = eventMove.getY();
            }

            if (Math.abs(z - eventMove.getZ()) > 0) {
                z = eventMove.getZ();
            }
        }
        ...
        else
        {
            ...
            double d4 = y;
            double d5 = z;
            Scaffold scaffoldModule = (Scaffold) Statics.getZeus().moduleManager.getModuleByClass(Scaffold.class);
            boolean flag = this.onGround && (this.isSneaking() || (scaffoldModule.isEnabled() && scaffoldModule.isSafeWalkEnabled())) && this instanceof EntityPlayer;
    ...
    public void moveFlying(float strafe, float forward, float friction)
    {
        ...
        if (this == Statics.getPlayer()) {
            ...
            cancelled = eventStrafe.isCancelled();
        }
     ...
     * Returns the distance to the entity. Args: entity
     */
    public float getDistanceToEntityX(Entity entityIn)
    {
        float f = (float)(this.posX - entityIn.posX);
        return MathHelper.sqrt_float(f * f);
    }

    /**
     * Returns the distance to the entity. Args: entity
     */
    public float getDistanceToEntityZ(Entity entityIn)
    {
        float f = (float)(this.posZ - entityIn.posZ);
        return MathHelper.sqrt_float(f * f);
    }
     ...
     * interpolated look vector
     */
    public Vec3 getLook(float yaw, float lastYaw, float pitch, float lastPitch, float partialTicks)
    {
        if (partialTicks == 1.0F)
        {
    ...
    public MovingObjectPosition rayTrace(double blockReachDistance, float yaw, float pitch)
    {
        ...
        Vec3 vec32 = vec3.addVector(vec31.xCoord * blockReachDistance, vec31.yCoord * blockReachDistance, vec31.zCoord * blockReachDistance);
        return this.worldObj.rayTraceBlocks(vec3, vec32, false);
    }
